...
def main():
    ...
    for task_name, ratio in output_paths:
        ...
        max_idx = df.groupby(['model', 'eff_method', 'hyperparam_1', 'hyperparam_2'])['maha accuracy'].transform(max) == df['maha accuracy']
        df_max = df[max_idx].groupby(['model', 'eff_method', 'hyperparam_1', 'hyperparam_2']).max()
        # df_max = df.groupby(['model', 'eff_method', 'hyperparam_1', 'hyperparam_2']).max(['maha accuracy'])
        ...
        with open(f'run_best_{task_name}_{ratio}.sh', 'w') as f:
            for idx, row in df_max.iterrows():
                ...
                master_port = 24998
                localhost='1,2'
                lr_scheduler = 'cosine_custom' if 'gpt-neo' in model else 'cosine'
                for seed in [1, 7, 123, 1234]:
                    run_command = f'deepspeed --include localhost:{localhost} --master_port {master_port} main_ood.py --overwrite_output_dir --task_name {task_name} --model_name_or_path {model} --output_dir outputs_best/{task_name}/{ratio}/{model} --ds_config ds_configs_samples/zero2_config.json --num_train_epochs 40 --cache_dir ~/data/model_data --lr {lr} --weight_decay 0.1 --lr_scheduler_type {lr_scheduler} --num_warmup_steps 200 --seed {seed} '
                    if ratio != 'full':
