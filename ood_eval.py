...
def evaluate_ood(args, method_name, model_engine, label_id_list, class_mean, 
                 class_var, norm_bank, features, ood, tag, tokenizer, config):
    ...
    dataloader = DataLoader(features, batch_size=args.per_device_eval_batch_size, 
    ...
    dataloader = DataLoader(ood, batch_size=args.per_device_eval_batch_size, 
                            collate_fn=collate_fn_partial)
    ...
    # if args.split:
    #     hist_dir = os.path.join(args.output_dir, model.name_or_path, f"{args.task_name}-{args.split_ratio}-seed{args.seed}", method_name, 'maha_histogram')
    # else:
    #     hist_dir = os.path.join(args.output_dir, model.name_or_path, f"{args.task_name}-no_split-seed{args.seed}", method_name, 'maha_histogram')
    ...
    for key in tqdm(keys, desc=f'Calculating OOD metrics'):
        ...
        if key == 'maha_acc':
            ...
            if key != 'maha':
                ...
                if args.split:
                    error_dir = os.path.join(args.output_dir, config.name_or_path, f"{args.task_name}-{args.split_ratio}-seed{args.seed}", method_name, f'Wrong_OOD({key})')
                else:
                    error_dir = os.path.join(args.output_dir, config.name_or_path, f"{args.task_name}-no_split-seed{args.seed}", method_name, f'Wrong_OOD({key})')
                ...
                with open(os.path.join(error_dir, "error_cases.tsv"), "w") as f:
                    ...
                    for fpr_i in fpr_indices[:total_error_cnt]:
                        score = scores[fpr_i]
                        input_ids = torch.LongTensor(ood[fpr_i - len(inl)]['input_ids']).unsqueeze(0).cuda()
                        attention_mask = torch.FloatTensor(ood[fpr_i - len(inl)]['attention_mask']).unsqueeze(0).cuda()
                        model_output = model_engine(input_ids=input_ids, attention_mask=attention_mask)
...
def prepare_ood(model_engine, dataloader, config):
...
def compute_ood(model, config, input_ids, label_id_list, class_mean, class_var, norm_bank, attention_mask=None, labels=None, ind=False, is_sigmoid=False, indices=None):
    ...
    logits = outputs[0]
    pooled = get_maha_embedding(input_ids, out_all_hidden, config)
